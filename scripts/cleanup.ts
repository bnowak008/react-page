  ...
  LEGACY_FILES.forEach(file => {
    ...
    if (existsSync(filePath)) {
      try {
        ...
        console.log(`${DRY_RUN ? '[DRY-RUN] Would remove' : '✓ Removed'} ${file}`);
      } catch (error) {
  ...
  LEGACY_DIRS.forEach(dir => {
    ...
    if (existsSync(dirPath)) {
      try {
        ...
        console.log(`${DRY_RUN ? '[DRY-RUN] Would remove' : '✓ Removed'} ${dir}`);
      } catch (error) {
...
function removeDeprecatedScripts() {
  ...
  deprecatedScripts.forEach(script => {
    if (pkg.scripts?.[script]) {
      ...
      console.log(`${DRY_RUN ? '[DRY-RUN] Would remove script' : '✓ Removed script'}: ${script}`);
      modified = true;
    }
  });
