...
describe('Performance Comparison: Redux vs Zustand', () => {
  ...
  describe('Store Initialization', () => {
    it('should compare initialization time', () => {
      ...
      console.log(
        `Zustand store initialization: ${zustandInitTime.toFixed(2)}ms`
      );
    ...
    it('should compare state update performance', () => {
      ...
      const reduxUpdateTime = measureExecutionTime(() => {
        for (let i = 0; i < 1000; i++) {
          reduxStore.dispatch({
            ...
            displayMode: i % 2 === 0 ? 'edit' : 'preview',
          });
        }
      });
      ...
      const zustandUpdateTime = measureExecutionTime(() => {
        for (let i = 0; i < 1000; i++) {
          zustandStore
            .getState()
            .setDisplayMode(i % 2 === 0 ? 'edit' : 'preview');
        }
      });

      console.log(`Redux 1000 state updates: ${reduxUpdateTime.toFixed(2)}ms`);
      console.log(
        `Zustand 1000 state updates: ${zustandUpdateTime.toFixed(2)}ms`
      );
  ...
  describe('Memory Usage', () => {
    it('should compare memory usage', () => {
      ...
      console.log(
        `Redux memory usage for 100 stores: ${(
          reduxMemoryUsage /
          1024 /
          1024
        ).toFixed(2)}MB`
      ...
      console.log(
        `Zustand memory usage for 100 stores: ${(
          zustandMemoryUsage /
          1024 /
          1024
        ).toFixed(2)}MB`
  ...
  describe('Selector Performance', () => {
    it('should compare selector performance', () => {
      ...
      console.log(
        `Redux 1000 selector calls: ${reduxSelectorTime.toFixed(2)}ms`
      );
      console.log(
        `Zustand 1000 selector calls: ${zustandSelectorTime.toFixed(2)}ms`
      );
